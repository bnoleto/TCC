...
public class Fogo_binarizado {
	...
	private static void salvarCSV(String arquivo, ArrayList<ArrayList<String>> tabela) {
		...
		try {
	        ...
	        bw.write("\"precipitacao\",\"temperatura\",\"umidade\",\"dias_de_secura\",\"houve_incendio\"");
	        bw.newLine();
	...
	public static void main (String[] args) {
		...
		ArrayList<ArrayList<String>> tabela = csv_to_ArrayList(pasta+arquivo, 1, 6);
		
		ArrayList<ArrayList<String>> lista_remover = new ArrayList<ArrayList<String>>();

		for(int i = 0; i < tabela.size(); i++) {
			
			tabela.get(i).remove(5); // remove rf			
		...
		for(int i = 0; i < tabela.size(); i++) {
			...
			try {
				...
				tabela.get(i).add(3,Double.toString(get_pse(i, tabela)));
		...
		System.out.println(tabela.get(20).get(4));
		
		normalizar(tabela);

		salvarCSV(pasta+"normalizado_bin_"+arquivo, tabela);
